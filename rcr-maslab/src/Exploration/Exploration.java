...
public class Exploration {
     ...
     */
    public Boolean InsertNewInformation(int time, StandardEntity ID, String Problem, int remainingLifeTime, int gravidadeIncendio) {
        ...
        if (Exploracao.containsKey(ID)) {
            ...
            if (!aux.get(0).equals(Problem)) {
                //Se foi identificado um novo problema, inclui na lista de mensagens a serem enviadas
                retorno = true;
            }
     ...
     *
     * @param HashMap
     * <StandardEntity, List> mensagem - Atualiza informações de exploração
     */
    ...
    public List<StandardEntity> GetBurningBuilds() {
        ...
        for (StandardEntity chave : chaves) {
            if (Exploracao.get(chave).get(2).toString().substring(1, 2)
                    .equals("1")) {
                Predios.add(chave);
            }
        }
    ...
    public List<StandardEntity> GetInjuredCivilians() {
        ...
        for (StandardEntity chave : chaves) {
            if (Exploracao.get(chave).get(2).toString().substring(0, 1)
                    .equals("1")) {
                civis.add(chave);
            }
        }
    ...
    public List<StandardEntity> GetBlockRoads() {
        ...
        for (StandardEntity chave : chaves) {
            if (Exploracao.get(chave).get(2).toString().substring(2, 3)
                    .equals("1")) {
                bloqueios.add(chave);
            }
        }
